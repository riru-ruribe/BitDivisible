...
""");
        ...
        var typeKind = "class";
        if (typeSymbol.TypeKind == TypeKind.Struct) typeKind = "struct";
...
{{accessibility}} partial {{typeKind}} {{className}}
{
{{sb}}
}
{{(isNamespace ? "}" : "")}}
...
""";
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    static string ToFirstLower(string s) => char.ToLower(s[0]) + s.Substring(1);
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    static string ToFirstUpper(string s) => char.ToUpper(s[0]) + s.Substring(1);
}
